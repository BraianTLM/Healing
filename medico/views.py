...
# se não for vai para tela de cadastro de médicos 
# e se for vai redireciona-lo para tela de abrir horario para as consultas
@login_required
def cadastro_medico(request):
    
    if vrf_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Você já está cadastrado como médico.')
        return redirect('/medicos/abrir_horario')
    
    if request.method == "GET":
        especialidades = Especialidades.objects.all()
        return render(request, 'cadastro_medico.html', {'especialidades': especialidades, 'vrf_medico': vrf_medico(request.user)})
...
# verifica se é médico e permite agendar um horário de serviço
@login_required
def abrir_horario(request):
    ...
    if request.method == "GET":
        ...
        return render(request, 'abrir_horario.html', {'dados_medicos': dados_medicos, 'datas_abertas': datas_abertas, 'vrf_medico': vrf_medico(request.user)})
    ...
    messages.add_message(request, constants.SUCCESS, 'Horário cadastro com sucesso.')
    return redirect('/medicos/abrir_horario/')
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        return render(request, 'consulta_area_medico.html', {'consulta': consulta, 'documentos': documentos, 'vrf_medico': vrf_medico(request.user)})
